  ...
  switch (head(t)) {
    ...
    case "application":
      return {
        ...
        args: list_to_array(map(ast_to_json, head(tail(tail(t))))).reverse() // microcode for app expects arg
    ...
    case "array_expression":
      return {
        tag: "arr_lit",
        elems: list_to_array(map(ast_to_json, head(tail(t)))).reverse() // microcode for arr_lit expects
        // expressions in reverse order
      };
...
const is_closure = (x) =>
  x !== null && typeof x === "object" && x.tag === "closure";

const is_builtin = (x) =>
  x !== null && typeof x === "object" && x.tag == "builtin";
...
const unop_microcode = {
  "-unary": (x) => -x,
  "!": (x) => (is_boolean(x) ? !x : error(x, "! expects boolean, found:"))
};
...
const builtin_mapping = {
  ...
  is_string: is_string,
  is_function: (x) =>
    typeof x === "object" && (x.tag == "builtin" || x.tag == "closure"),
...
const microcode = {
  ...
  unop: (cmd) => push(C, { tag: "unop_i", sym: cmd.sym }, cmd.frst),
  binop: (cmd) => push(C, { tag: "binop_i", sym: cmd.sym }, cmd.scnd, cmd.frst),
  log: (cmd) =>
    push(
      C,
      cmd.sym == "&&"
  ...
  assmt: (cmd) => push(C, { tag: "assmt_i", sym: cmd.sym }, cmd.expr),
  lam: (cmd) =>
    push(S, { tag: "closure", prms: cmd.prms, body: cmd.body, env: E }),
  arr_lit: (cmd) =>
    push(C, { tag: "arr_lit_i", arity: cmd.elems.length }, ...cmd.elems), // already in reverse order, see ast_to_json
  arr_acc: (cmd) => push(C, { tag: "arr_acc_i" }, cmd.ind, cmd.arr),
  arr_assmt: (cmd) =>
    push(C, { tag: "arr_assmt_i" }, cmd.expr, cmd.ind, cmd.arr),
  arr_len: (cmd) => push(C, { tag: "arr_len_i" }, cmd.expr),

  //
  // statements
  //
  seq: (cmd) => push(C, ...handle_sequence(cmd.stmts)),
  cond_stmt: (cmd) =>
    push(C, { tag: "branch_i", cons: cmd.cons, alt: cmd.alt }, cmd.pred),
  blk: (cmd) => {
    const locals = scan(cmd.body);
    const unassigneds = locals.map((_) => unassigned);
    if (!(C.length === 0)) push(C, { tag: "env_i", env: E });
    push(C, cmd.body);
  ...
  ret: (cmd) => push(C, { tag: "reset_i" }, cmd.expr),
  fun: (cmd) =>
    push(C, {
      tag: "const",
      sym: cmd.sym,
  ...
  branch_i: (cmd) => push(C, S.pop() ? cmd.cons : cmd.alt),
  while_i: (cmd) =>
    S.pop()
      ? push(
          C,
          cmd, // push while_i itself back on control
          cmd.pred,
...
const all_except_last = (xs) =>
  is_null(tail(xs)) ? null : pair(head(xs), all_except_last(tail(xs)));
...
const debug = (cmd) => {
  ...
  display("", "C:");
  for (let cmd of C) display("", command_to_string(cmd));
  display("", "S:");
  for (let val of S) display("", value_to_string(val));
  display("", "E:");
  for_each((frame) => {
